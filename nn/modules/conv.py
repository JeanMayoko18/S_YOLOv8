...
__all__ = ('Conv', 'Conv2', 'LightConv', 'DWConv', 'DWConvTranspose2d', 'ConvTranspose', 'Focus', 'GhostConv',
           'ChannelAttention', 'SpatialAttention', 'CBAM', 'Concat', 'RepConv', 'CNeB','CNeBV2', 'ConvNextBlock', "ConvNextBlockV2"
           'MHSA', 'CARAFE', 'DownsampleLayer', 'SimSPPF', 'SimCSPSPPF', 'CSPSPPFModule', 'BoT3')
...
class CNeBV2(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)
        self.m = nn.Sequential(*(ConvNextBlockV2(c_) for _ in range(n)))
    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))
...
class DownsampleLayer(nn.Module):
    def __init__(self, size, scale_factor, mode='nearest'):
        """
        Initialize the DownsampleLayer.

        Args:
            scale_factor (float or int): The scale factor for downsampling.
            mode (str): The interpolation mode. Options are 'nearest' (default), 'linear', 'bilinear', 'bicubic', 'trilinear'.
        """
    ...
    def forward(self, x):
        """
        Forward pass of the DownsampleLayer.
        ...
        """
        # Use torch.nn.functional.interpolate to downsample the input tensor
        x_downsampled = F.interpolate(x, size=self.size, scale_factor=self.scale_factor, mode=self.mode)
...
class CSPSPPFModule(nn.Module):
    ...
    def __init__(self, in_channels, kernel_size=5, e=0.5,):
        super().__init__()
        out_channels = in_channels
...
class SimCSPSPPF(nn.Module):
    ...
    def __init__(self, in_channels, kernel_size=5, e=0.5,):
        super().__init__()
        self.cspsppf = CSPSPPFModule(in_channels, kernel_size, e,)
